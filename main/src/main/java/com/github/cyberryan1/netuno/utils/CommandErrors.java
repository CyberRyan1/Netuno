...
public class CommandErrors {
    ...
    public static void sendBanLengthTooLarge( CommandSender sender ) {
        long maxBanLengthTimestamp = TimestampUtils.getTimestampFromUnformulatedLength( YMLUtils.getConfig().getStr( "ban.max-time-length" ) );
        CyberMsgUtils.sendMsg( sender, "&sYou can only ban for a maximum of " +
                TimestampUtils.timestampToFormulatedLength( maxBanLengthTimestamp, -1 ) );
    }
}