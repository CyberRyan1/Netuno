...
public class BanPunishGUI {
    ...
    public BanPunishGUI( Player staff, OfflinePlayer target ) {
        this.staff = staff;
        this.target = target;
        this.punishButtons = PunishSettings.BAN_BUTTONS.multiButton();

        this.rowCount = determineRowCount();
        this.gui = new Gui( PunishSettings.BAN_INVENTORY_NAME.coloredString().replace( "[TARGET]", target.getName() ),
