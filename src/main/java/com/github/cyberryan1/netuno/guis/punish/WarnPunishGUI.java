...
public class WarnPunishGUI {
    ...
    public WarnPunishGUI( Player staff, OfflinePlayer target ) {
        this.staff = staff;
        this.target = target;
        this.punishButtons = PunishSettings.WARN_BUTTONS.multiButton();

        this.rowCount = 3 + ( this.punishButtons.getButtons().size() / 9 );
        this.gui = new GUI( PunishSettings.WARN_INVENTORY_NAME.coloredString().replace( "[TARGET]", target.getName() ),
