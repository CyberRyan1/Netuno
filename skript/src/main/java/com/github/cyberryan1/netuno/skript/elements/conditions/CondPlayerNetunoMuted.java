...
public class CondPlayerNetunoMuted  extends RegularCondition {
    ...
    public boolean check( Event event ) {
        ...
        if ( p.isOnline() )
            return NetunoSkript.SERVICE.getPlayerNow( p.getPlayer() ).get().isPunished( ApiPunishment.PunType.MUTE ) ? isNegated() : !isNegated();
        else
            return NetunoSkript.SERVICE.getPlayer( p ).join().isPunished( ApiPunishment.PunType.MUTE ) ? isNegated() : !isNegated();
    }
