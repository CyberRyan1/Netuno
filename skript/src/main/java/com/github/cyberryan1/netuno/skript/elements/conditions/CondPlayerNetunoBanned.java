...
public class CondPlayerNetunoBanned extends RegularCondition {
    ...
    public boolean check( Event event ) {
        ...
        if ( p.isOnline() )
            return NetunoSkript.SERVICE.getPlayerNow( p.getPlayer() ).get().isPunished( ApiPunishment.PunType.BAN ) ? isNegated() : !isNegated();
        else
            return NetunoSkript.SERVICE.getPlayer( p ).join().isPunished( ApiPunishment.PunType.BAN ) ? isNegated() : !isNegated();
    }

    @Override
    public String toString( Event event, boolean debug ) {
        return "%offlineplayer% (1¦is|2¦is(n't¦ not)) [currently] netuno banned" + player.toString( event, debug );
    }
}